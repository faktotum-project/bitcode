...
|---|---|---|
| **0 — Ricognizione** | ~~Verifiche V1–V8~~ **chiusa il 2026-08-27**, verificata contro binari reali (non solo doc/tarball npm): il daemon è `waved`/`wavecli`, progetto Go separato (`github.com/lightninglabs/wavelength`), distribuito come binari precompilati **firmati** per piattaforma via GitHub Releases (non un submodule da compilare come CDK). **Scoperta chiave**: `wavecli mcp serve` è un server MCP nativo che espone 17 metodi del daemon con classificazione `side_effect` già inclusa — risolve il residuo "MCP nativo" ed elimina la necessità di un client gRPC/REST scritto a mano | Sezioni 3–4 senza incognite ✅ |
| **1 — Engine + read-only su signet** | **Consegnato (2026-08-27), verificato end-to-end contro un `waved` reale su signet** (non solo mock): `scripts/build-wavelength.sh` scarica e verifica-checksum i binari ufficiali `v0.1.1` in `deps/wavelength/bin/` (Opzione B, ma via fetch firmato invece che build da sorgente — non serve toolchain Go); `src/wavelength/daemon.mjs` gestisce il ciclo di vita di `waved` (spawn/stop, drain di stdout/stderr per evitare deadlock sul buffer della pipe, log tail per diagnostica); `src/wavelength/network.mjs` esteso con la config RPC locale (macaroon+TLS sempre attivi, gateway HTTP disattivato); `src/wavelength/tools.mjs` sostituisce il vecchio confine `engine.mjs`/`FACADE_METHODS` con: avvio del daemon → attesa readiness (poll TCP reale, non solo esistenza file) → connessione a `wavecli mcp serve` via il client MCP già esistente (`src/mcp.mjs`) → filtro automatico ai soli metodi `mcp_tool && !side_effect` (da `wavecli schema --all --json`, offline). Risultato: 8 tool `wl_*` (non più solo 2), wiring async in `loadExtensions()` (cli.mjs) invece che in `buildTools()`. `tests/wavelength.test.mjs` (14 test, nessuno richiede il daemon reale). **Verificato dal vivo**: `wl_getinfo` risponde con dati reali dal daemon; `wl_daemon_balance`/`wl_ark_vtxos_list`/`wl_send_prepare`/`wl_exit_status` rispondono a livello RPC (richiedono solo un wallet, non ancora creato — Fase 2). **Residuo nuovo**: 3 tool (`wl_activity`, `wl_balance`, `wl_exit_summary`) rispondono `daemon was not built with -tags wavewalletrpc` — il binario ufficiale precompilato non include quel build tag; sbloccarli richiede una build da sorgente con toolchain Go (`wavelength-source-v0.1.1.tar.gz` + `make build-wavewalletrpc`), da valutare in una fase successiva | `wl_getinfo` verde su signet ✅ — fatto |
