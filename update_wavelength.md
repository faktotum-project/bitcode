...
| **1 — Engine + read-only su signet** | **In corso (2026-07-22):** consegnati `src/wavelength/` (`network.mjs` con preset verificati e guard mainnet/regtest, `engine.mjs` come confine sui verbi facade del daemon con transport innestabile, `tools.mjs` con `wl_info`/`wl_balance`), wiring in `buildTools` e nel system prompt, `tests/wavelength.test.mjs` (10 test, transport mockato). **Manca il transport reale**: `scripts/build-wavelength.sh` (Opzione B) o shim Node (Opzione C) resta bloccato sul residuo di Fase 0 | `wl_info` verde su signet in CI locale (richiede il transport; senza, i tool falliscono con errore actionable) |
