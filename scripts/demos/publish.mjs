...
const gallery = `<!-- RECORDED DEMOS START -->
    <section class="section recorded-demos" id="demos" aria-labelledby="demos-title">
      <div class="container">
        <div class="section-heading"><div><p class="eyebrow">See the work happen</p><h2 class="section-title" id="demos-title">Install. Configure.<br>Put it to work.</h2></div><p class="section-description">Three terminal walkthroughs, from setup to your first useful answer.</p></div>
        ${manifest.demos.map(d=>{const c=descriptions[d.id];return `<article class="demo-story" aria-labelledby="heading-${d.id}"><div class="demo-story-media">${video(d,`recording-${d.id}`)}<div class="recording-meta"><span>${duration(d.exports['16x9'].duration)} · CLI walkthrough</span><span>${d.recordedAt.slice(0,10)}</span></div></div><div class="demo-story-copy"><p class="eyebrow">${c.eyebrow}</p><h3 id="heading-${d.id}">${c.name}</h3><p>${c.body}</p><div class="recording-model">${c.context}</div></div></article>`;}).join('\n')}
...
if(!html.includes('<!-- RECORDED DEMOS START -->')) throw new Error('Missing page integration anchors.');
